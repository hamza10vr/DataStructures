...
int main()
{
    ...
    // reading the first number from file as  string and storing in linked list as single digits
    std::string line;                 
    inFS >> line;
    unsigned int i;
...
{
    // prints the total sum using range based for-loop
    std::cout << "The sum is: ";
    ...
    // prints the fist 10 digits, using iterator
...
// reverses the string before feeding it to the list for addition.
...
void addition (std::list<int>& list_1, std::string new_line)
{
    ...
    // iterates through each node of list and adding the new number read from file
    for (it = list_1.begin(); it != list_1.end(); ++it)
    {
        ...
        // if the new number after addition is greater than 10 
        // store the number in the list as follows
        if (*it >= 10)          // ex 14
        {
            carry = 1;          // carry  = 1
    ...
    // once the calculation is finished and still a carrry. 
    if (carry == 1)
    {
